...
for (const viewport of [{ width: 1280, height: 900 }, { width: 390, height: 844 }]) {
  test(`landing works at ${viewport.width}px without remote resources`, async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Download for Windows' })).toHaveAttribute('href', 'https://github.com/burkeholland/transcribe-me/releases/download/v0.3.0/TranscribeMe-0.3.0-windows-x64.zip');
    await expect(page.getByRole('link', { name: 'SHA-256 checksums' })).toHaveAttribute('href', 'https://github.com/burkeholland/transcribe-me/releases/download/v0.3.0/SHA256SUMS.txt');
